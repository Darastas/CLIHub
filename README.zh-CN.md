...
- **原生 Win32 剪贴板直写与全能选区复制**：
  - 接入 Win32 原生剪贴板 API（UTF-16），彻底消除跨应用复制偶发丢失与延迟问题。
  - 支持 QuickEdit 模式：鼠标选中文本释放即自动复制到系统剪贴板，支持单行、多行、反向选择以及中文宽字符（Wide Char）完整提取。
  - 完善快捷键体系：支持 `Ctrl+Shift+C` / `Cmd+Shift+C` / `Ctrl+Insert` 终端标准复制，`Ctrl+Shift+V` / `Cmd+Shift+V` / `Shift+Insert` 粘贴。
  - 经典右键交互：有选区时右键一键复制并取消选中，无选区时右键直接粘贴。
- **后台窗口焦点保护与中断信号隔离**：
  - 严格守卫操作系统窗口焦点，当应用处于后台或非激活状态时，绝对不拦截或转发任何外部快捷键（如在其他软件中按 `Ctrl+C`），避免子进程被误杀。
  - 恢复常规 `Ctrl+C` 为标准的终端 `SIGINT` 中断退出信号。
- **修复子进程退出无限刷屏 Bug**：
  - 修复进程退出后 `pty.child.try_wait()` 每帧重复打印 `[process exited]` 的死循环问题。
- **全架构原生预编译包**：提供 Windows x64 (x86_64)、x86 (32-bit)、ARM64 三架构原生 Release 二进制文件。
