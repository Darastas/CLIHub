...
- 🤖 **Agent Chat 多智能体协作工作流** — 原生多 AI CLI 协同中枢（Codex、Claude 等）：
  ...
  - **全自动信箱交接协议 (Mailbox Protocol & CLI Tool)**：随附 `clihub-agent.exe` 独立信箱 CLI 工具与文件协议（`inbox`、`send`、`list`），AI Agent 能够自主拉取任务、接收上下文、提交任务产物并触发自动下一棒交接。
  - **MCP Server 兼容网桥**：内置标准 Model Context Protocol (MCP) 服务器实现，为兼容的 AI 客户端提供直接的信箱工具调用接口。
...
- **Agent Chat 多智能体协作工作流 (Multi-Agent Collaboration Engine)**：
  - 原生嵌入式多终端卡片：为多 AI 角色分配基于 Windows ConPTY 的交互进程，Alacritty 状态机 + egui 渲染，支持并排卡片展示、独立输入聚焦与滚轮历史翻阅。
  - 下沉式流式指令输入框 (Sunken Composer)：延续阴阳刻极简设计语言，动态自适应多行展开，支持快捷选择首位执行者与一键下发。
  - 全自动信箱交接协议：随附 `clihub-agent.exe` 独立信箱 CLI 工具与文件交接协议（`inbox`、`send`、`list`），实现 AI 智能体间全自动任务接力。
  - 内置标准 MCP Server 桥接网桥：为兼容客户端提供标准化的 Model Context Protocol 工具调用集成。
  - 彻底修复文本框与终端焦点冲突：打字时严格独占键盘输入，杜绝终端抢夺焦点的跳变问题。
  - 极简界面清理：移除左上角多余加号与侧边栏底部的空态占位提示。
  - 搜索栏动态平滑展开动效：`Ctrl+F` 呼出与隐藏时加入丝滑展开动画，完全对齐终端原生视觉风格。
