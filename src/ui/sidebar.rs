...
pub fn show(ui: &mut Ui, sessions: &[Session], selected: usize) -> SidebarAction {
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(RichText::new("AI CLI Hub").size(15.0).color(TEXT));
    });
    ui.add_space(2.0);
    ui.horizontal(|ui| {
        ui.add_space(34.0);
        ui.label(RichText::new("Terminal Hub").size(11.0).color(MUTED));
    });
    ui.add_space(10.0);
    ui.separator();
    ui.add_space(12.0);
    ...
    ui.horizontal(|ui| {
        ui.add_space(2.0);
        ui.label(
            RichText::new(format!("{} session(s) · keep running in background", sessions.len()))
                .size(10.0)
                .color(MUTED),
        );
    });
